...
.brand{font-weight:800;letter-spacing:.06em}.sub{color:#8292ad;font-size:12px}.page{max-width:1180px;margin:auto;padding:32px 24px}.hero{margin:18px 0 28px}
...
async function getManifest() {
  ...
  if (!response.ok) throw new Error('Unable to load offer manifest: ' + response.status)
  const payload = await response.json()
  if (!Array.isArray(payload?.offers)) throw new Error('Manifest must contain offers[]')
...
function loadRemote(entry) {
  ...
  if (remoteLoads.has(entry.id)) return remoteLoads.get(entry.id)
...
function MountedOffer({ entry, remote, onBack, notify }) {
  ...
  return (
    <main className="page">
      <button className="btn back" onClick={onBack}>← All offers</button>
      <div className="tag">Runtime remote · {entry.remoteUrl}</div>
    ...
    <div className="app">
      <header className="top">
        <div>
          ...
          <div className="sub">Webpack · runtime remote bundles</div>
        </div>
        <div className="sub">No offer source imports</div>
      </header>
      ...
      {activeEntry && activeRemote ? (
        ...
        <main className="page">
          <section className="hero">
            <div className="tag">Independent repositories</div>
            <h1>Runtime Offer Registry</h1>
            <p>
              The shell loads only a manifest and remote JavaScript URLs.
              Each offer owns its React runtime and builds to one dist/offer.js file.
            </p>
          </section>
            ...
            {entries.map((entry) => {
              ...
              return (
                <article className="card" key={entry.id}>
                  <div>
                    <div className="id">{entry.id} · offer.js</div>
